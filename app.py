...
def get_mood_tags(mood_text):
    """Extract keyword tags from mood text"""
    tag_keywords = [
    ...
    found = [t for t in tag_keywords if t in mood_lower]
...
def mood_score(btc_vol, spy_vol):
    """Simple 0-10 mood score: low vol = calm (1-4), high vol = tense (7-10)"""
...
def render_ui(state):
    ...
    if state == "idle":
        ...
        <div class="metric-card">
            ...
            <div class="metric-change neutral">5d avg</div>
        </div>
        <div class="metric-card">
        ...
        btn_label = "Generate today's track"
        btn_icon = "▶"

    else:
        btc_vol = st.session_state['btc_vol']
        spy_vol = st.session_state['spy_vol']
        btc_change = st.session_state['btc_change']
        spy_change = st.session_state['spy_change']
        mood = st.session_state['mood']
        tags = st.session_state['tags']
        track_file, track_name, track_icon, track_dur = st.session_state['track']
        score, score_label = st.session_state['score']
        ...
        <div class="metric-card">
            ...
            <div class="metric-change neutral">{score_label}</div>
        </div>"""
        ...
        <div class="track-card">
            ...
            <div class="track-info">
                ...
                <div class="wave-row">
                    <div class="waveform">
                        {"".join(f'<div class="wave-bar" style="height:{h}px;animation-delay:{d}s"></div>' for h, d in [(18,0),(26,0.1),(14,0.2),(28,0.05),(20,0.15),(24,0.25),(16,0.08),(22,0.18),(12,0.3),(26,0.12),(20,0.22),(18,0.07)])}
                    </div>
                </div>
            </div>
        </div>
...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .live-badge {{
    ...
    background: rgba(45,156,106,0.08);
    border: 0.5px solid rgba(45,156,106,0.2);
    border-radius: 20px;
    padding: 4px 10px;
    font-size: 11px;
    font-family: 'DM Mono', monospace;
  ...
  .up {{ color: #2d9c6a; }}
  .down {{ color: #c0392b; }}
  .neutral {{ color: rgba(255,255,255,0.25); }}
  .neutral-val {{ color: rgba(255,255,255,0.25); }}
  ...
  .mood-header {{ display: flex; align-items: flex-start; gap: 12px; }}
  .mood-text {{
    font-size: 14px;
  ...
  .wave-bar {{
    width: 2.5px;
    border-radius: 2px;
    background: rgba(89,144,255,0.4);
  ...
  .btn-generate {{
    width: 100%;
    padding: 14px;
    background: rgba(89,144,255,0.08);
    border: 0.5px solid rgba(89,144,255,0.2);
    border-radius: 10px;
    color: rgba(89,144,255,0.8);
    font-size: 14px;
    font-family: 'DM Sans', sans-serif;
    font-weight: 500;
    cursor: pointer;
  ...
  .btn-generate:hover {{
    background: rgba(89,144,255,0.14);
    border-color: rgba(89,144,255,0.35);
  }}
...
<div class="container">
  ...
  <form method="get">
    <button type="submit" name="generate" value="1" class="btn-generate">
      <span class="btn-icon">{btn_icon}</span>
      {btn_label}
    </button>
  </form>
...
if st.session_state.get('generated'):
    html = render_ui("ready")
    components.html(html, height=720, scrolling=False)
    st.audio(st.session_state['track'][0])
else:
    html = render_ui("idle")
